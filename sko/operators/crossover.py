...
def crossover_1point(self):
    ...
    selected_crossover = 2*np.random.choice(self.size_pop//2, int(self.size_pop//2 * crossover_rate), replace=False)
    for i in selected_crossover:
        # print(i)
        if i == self.FitV.argmax():
            continue
        n = np.random.randint(0, self.len_chrom, 1)
        # crossover at the point n
        seg1, seg2 = self.Chrom[i, n[0]].copy(), self.Chrom[i + 1, n[0]].copy()
        self.Chrom[i, n[0]], self.Chrom[i + 1, n[0]] = seg2, seg1
    return self.Chrom
